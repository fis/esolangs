...
(function (){
    ...
    TapePlot.setup = function (opts) {
        ...
        var polItem = ProgPlot.form
            .append('div').attr('class', 'form-group')
            .append('div').attr('class', 'form-check');
        TapePlot.polCheck = polItem.append('input')
            .attr('type', 'checkbox').attr('id', 'pol').attr('class', 'form-check-input')
            .on('change', onPolChange);
        polItem.append('label')
            .attr('for', 'pol').attr('class', 'form-check-label')
            .node().appendChild(document.createTextNode('Show sieve/kettle polarity separately'));
    ...
    function makeDataScale(min, max) {
        ...
        var scale = TapePlot.logScale ? d3.scaleLog() : d3.scaleLinear();
