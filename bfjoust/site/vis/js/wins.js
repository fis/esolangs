...
(function (){
    ...
    function setup() {
        ...
        for (var o = 0; o < orders.length; o++) {
            var orderItem = orderGroup.append('div').attr('class', 'form-check form-check-inline');
            orderItem.append('input')
                .attr('type', 'radio').attr('class', 'form-check-input')
                .attr('name', 'ord').attr('id', 'ord'+o).attr('value', o)
                .on('change', onOrdChange);
            var orderLabel = orderItem.append('label')
                .attr('for', 'ord'+o).attr('class', 'form-check-label');
    ...
    function updatePlot() {
        ...
        rows.exit().remove();
        rows = rows.enter().append('g').attr('class', 'row').merge(rows);
        rows.attr('transform', rowCoord);
        ...
        points
          .enter().append('text').attr('class', 'p')
        ...
        names
          .enter().append('text').attr('class', 'l')
