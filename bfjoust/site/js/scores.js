...
function makeHead(sort, label, remove)
{
    ...
    var sortAsc = $('<span class="sorticon oi oi-arrow-thick-bottom"></span>');
    var sortDesc = $('<span class="sorticon oi oi-arrow-thick-top"></span>');
    ...
    if (remove)
    {
        var del = $('<span class="sorticon oi oi-x"></span>');
...
function build()
{
    ...
    for (var i = 0; i < Zhill.scoringMethodOrder.length; i++)
    {
        ...
        var btn = $('<button type="button" class="btn btn-default btn-sm"><span class="oi oi-plus"></span> '+label+'</button>');
